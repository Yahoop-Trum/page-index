...
def toc_detector_single_page(content, model=None):
    ...
    result = json_content.get('toc_detected', 'no')
    if not result:
        logging.warning(f"toc_detected missing from JSON response. Using default 'no'. Response was: {response}")
        result = 'no'
    return result
...
def check_if_toc_extraction_is_complete(content, toc, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
    result = json_content.get('completed', 'no')
    if not result:
        logging.warning(f"'completed' key missing from JSON response. Using default 'no'. Response was: {response}")
        result = 'no'
    return result
...
def check_if_toc_transformation_is_complete(content, toc, model=None):
    ...
    result = json_content.get('completed', 'no')
    if not result:
        logging.warning(f"'completed' key missing in transformation check. Using default 'no'. Response was: {response}")
        result = 'no'
    return result
...
def detect_page_index(toc_content, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
    result = json_content.get('page_index_given_in_toc', 'no')
    if not result:
        logging.warning(f"'page_index_given_in_toc' key missing from JSON response. Using default 'no'. Response was: {response}")
        result = 'no'
    return result
...
def toc_transformer(toc_content, model=None):
    ...
    if if_complete == "yes" and finish_reason == "finished":
        ...
        if isinstance(last_complete, list):
            return convert_page_to_int(last_complete)
        logging.error(f"toc_transformer: expected 'table_of_contents' but got: {last_complete}")
        return []
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        if attempt > max_attempts:
            logging.error(
                'toc_transformer failed after max retries. last_complete=%s new_complete=%s',
                last_complete,
                new_complete,
            )
        ...
        new_complete = new_complete.strip()
        if new_complete:
            last_complete = last_complete + new_complete
        else:
            logging.warning(f"toc_transformer: received empty continuation on attempt {attempt}")
        ...
        for page_item in toc_page:
            if phy_item.get('title') == page_item.get('title'):
                ...
                if physical_index is not None:
                    try:
                        # Extract numeric part if physical_index contains non-numeric characters
                        import re
                        numeric_part = re.findall(r'\d+', str(physical_index))
                        if numeric_part:
                            physical_index_int = int(numeric_part[0])
                        ...
                        # Skip entries with invalid physical indices
                        pass
    return pairs
...
def generate_toc_init(part, model=None):
    ...
    if finish_reason == 'finished':
         return normalize_json_list(extract_json(response))
    else:
        raise Exception(f'finish reason: {finish_reason}')
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    ...
    toc_with_page_number = normalize_json_list(generate_toc_init(group_texts[0], model))
    for group_text in group_texts[1:]:
        toc_with_page_number_additional = normalize_json_list(generate_toc_continue(toc_with_page_number, group_text, model))    
...
def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=None, model=None, logger=None):
    try:
        ...
        if logger:
            logger.error(f"toc_transformer failed, falling back to no-page-number mode: {e}")
...
async def single_toc_item_index_fixer(section_title, content, model=None):
    ...
    response = await llm_acompletion(model=model, prompt=prompt)
    json_content = extract_json(response)
    physical_index = json_content.get('physical_index', None)
    if physical_index is None:
        logging.warning(f"'physical_index' key missing from JSON response. Response was: {response}")
        return None
