...
async def generate_summaries_for_structure_md(structure, summary_token_threshold, model=None, max_concurrent=5):
    """Generate summaries with concurrency limiting"""
    nodes = structure_to_list(structure)
    
    # Filter nodes that need summaries based on token threshold
    summary_tasks = []
    node_indices = []
    import hashlib
    ...
    for idx, node in enumerate(nodes):
        ...
        if num_tokens >= summary_token_threshold:
            ...
            summary_tasks.append((idx, node, text_hash))
            node_indices.append(idx)
    
    # Limit concurrency using semaphore
    semaphore = asyncio.Semaphore(max_concurrent)
    
    async def bounded_generate_summary(idx, node, cache_key):
        async with semaphore:
            from utils import generate_node_summary as gen_summary
            return idx, await gen_summary(node, model=model, cache_key=cache_key)
    
    if summary_tasks:
        results = await asyncio.gather(*[
            bounded_generate_summary(idx, node, text_hash) 
            for idx, node, text_hash in summary_tasks
        ])
        
        # Apply summaries back to nodes
        for idx, summary in results:
            if not nodes[idx].get('nodes'):
                nodes[idx]['summary'] = summary
            else:
                nodes[idx]['prefix_summary'] = summary
